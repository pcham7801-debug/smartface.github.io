...
<div class="row mb-4">
    <div class="col-md-8">
        <h3 class="fw-bold text-dark"><i class="fa-solid fa-list-check text-primary me-2"></i> Enrollment Management</h3>
        <p class="text-muted">Assign students to subject schedules and manage enrolled class rosters</p>
    </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($enrollments as $row): ?>
                            <tr>
                                ...
                                <td><span class="badge bg-light text-dark border"><?= htmlspecialchars($row['course']) ?> - <?= htmlspecialchars($row['year_level']) ?></span></td>
                                <td class="fw-bold"><?= htmlspecialchars($row['subject_code']) ?></td>
                                <td><?= htmlspecialchars($row['subject_name']) ?></td>
                                <td><?= date('M d, Y', strtotime($row['enrolled_at'])) ?></td>
                                <td class="text-end pe-3">
                                    <a href="?drop_id=<?= $row['enrollment_id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Drop student from this subject?')">
                                        <i class="fa-solid fa-user-minus"></i> Drop
                                    </a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="student_id" class="form-select" required>
                            ...
                            <?php foreach ($studentsList as $stu): ?>
                                <option value="<?= $stu['id'] ?>" <?= $selectedStudentId === intval($stu['id']) ? 'selected' : '' ?>>
